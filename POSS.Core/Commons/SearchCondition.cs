...
namespace WHC.Framework.Commons
{
    ...
    public class SearchCondition
    {
        ...
        {
            this.conditionTable.Add(this.conditionTable.Count+1/*fielName*/, new SearchInfo(fielName, fieldValue, sqlOperator));
            return this;
        }
        ...
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldValue, sqlOperator, excludeIfEmpty));
            return this;
        }
        ...
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldValue, sqlOperator, excludeIfEmpty, groupName));
            return this;
        } 

        #endregion

        #region 添加查询条件(有字段中文说明)
        ...
        /// 用法一：
        /// SearchCondition searchObj = new SearchCondition();
        /// searchObj.AddCondition("Test","测试字段", 1, SqlOperator.NotEqual);
        /// searchObj.AddCondition("Test2","测试字段2", "Test2Value", SqlOperator.Like);
        /// string conditionSql = searchObj.BuildConditionSql();
        /// 
        /// 用法二：AddCondition函数可以串起来添加多个条件
        /// SearchCondition searchObj = new SearchCondition();
        /// searchObj.AddCondition("Test","测试字段", 1, SqlOperator.NotEqual).AddCondition("Test2","测试字段2", "Test2Value", SqlOperator.Like);
        /// string conditionSql = searchObj.BuildConditionSql();
        /// </example>
        /// </summary>
        /// <param name="fielName">字段名称</param>
        ...
        /// <param name="sqlOperator">SqlOperator枚举类型</param>
        /// <returns>增加条件后的Hashtable</returns>
        public SearchCondition AddCondition(string fielName,string fieldDescribe ,object fieldValue, SqlOperator sqlOperator)
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldDescribe, fieldValue, sqlOperator));
            return this;
        }
        ...
        /// <param name="excludeIfEmpty">如果字段为空或者Null则不作为查询条件</param>
        /// <returns></returns>
        public SearchCondition AddCondition(string fielName, string fieldDescribe, object fieldValue, SqlOperator sqlOperator, bool excludeIfEmpty)
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldDescribe, fieldValue, sqlOperator, excludeIfEmpty));
            return this;
        }
        ...
        /// <param name="groupName">分组的名称，如需构造一个括号内的条件 ( Test = "AA1" OR Test = "AA2"), 定义一个组名集中条件</param>
        /// <returns></returns>
        public SearchCondition AddCondition(string fielName,string fieldDescribe, object fieldValue, SqlOperator sqlOperator,bool excludeIfEmpty, string groupName)
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldDescribe, fieldValue, sqlOperator, excludeIfEmpty, groupName));
            return this;
        }
        
        #endregion

        #region 添加查询条件(有字段中文说明且有字段值中文描述)
        ...
        /// <param name="fieldDescribe">字段中文描述</param>
        /// <param name="fieldValue">字段值</param>
        /// <param name="fieldValueDescribe">字段值描述</param>
        /// <param name="sqlOperator">SqlOperator枚举类型</param>
        /// <returns>增加条件后的Hashtable</returns>
        public SearchCondition AddCondition(string fielName, string fieldDescribe, object fieldValue,string fieldValueDescribe, SqlOperator sqlOperator)
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldDescribe, fieldValue,fieldValueDescribe, sqlOperator));
            return this;
        }
        ...
        /// <param name="excludeIfEmpty">如果字段为空或者Null则不作为查询条件</param>
        /// <returns></returns>
        public SearchCondition AddCondition(string fielName, string fieldDescribe, object fieldValue,string fieldValueDescribe, SqlOperator sqlOperator, bool excludeIfEmpty)
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldDescribe, fieldValue, fieldValueDescribe, sqlOperator, excludeIfEmpty));
            return this;
        }
        ...
        /// <param name="groupName">分组的名称，如需构造一个括号内的条件 ( Test = "AA1" OR Test = "AA2"), 定义一个组名集中条件</param>
        /// <returns></returns>
        public SearchCondition AddCondition(string fielName, string fieldDescribe, object fieldValue, string fieldValueDescribe, SqlOperator sqlOperator, bool excludeIfEmpty, string groupName)
        {
            this.conditionTable.Add(this.conditionTable.Count + 1/*fielName*/, new SearchInfo(fielName, fieldDescribe, fieldValue, fieldValueDescribe, sqlOperator, excludeIfEmpty, groupName));
            return this;
        }
        ...
        /// <summary>
        /// 根据对象构造相关的不带WHERE条件语句（指定数据库类型），如返回的语句是:
        /// <![CDATA[
        /// Test4  <  'Value4' AND Test6  >=  'Value6' AND Test7  <=  'value7' AND Test  <>  '1' AND Test5  >  'Value5' AND Test2  Like  '%Value2%' AND Test3  =  'Value3'
        /// ]]>
        /// </summary>
        /// <returns></returns> 
        public string BuildConditionSqlNoWhere(DatabaseType dbType)
        {
            ...
            foreach (DictionaryEntry de in this.conditionTable)
            {
                ...
                if (string.IsNullOrEmpty(searchInfo.GroupName))
                {
                    ...
                    {
                        sb.AppendFormat(" AND {0} {1} {2}", searchInfo.FieldName,
                            this.ConvertSqlOperator(searchInfo.SqlOperator), string.Format("({0})", searchInfo.FieldValue.ToString().BuildSqlString(',')).RemoveSpaces());
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.LikeEndAt)
                    {
                        sb.AppendFormat(" AND {0} {1} '{2}'", searchInfo.FieldName,
        ...
        /// </summary>
        /// <returns></returns> 
        public string BuildConditionDescribe()
        {
            if (this.conditionTable.Count == 0) return string.Empty;
            ...
            foreach (DictionaryEntry de in this.conditionTable)
            {
                ...
                if (string.IsNullOrEmpty(searchInfo.GroupName))
                {
                    if (searchInfo.SqlOperator == SqlOperator.Like)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 包含 『{1}』",searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.NotLike)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 不包含 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.LikeStartAt)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 以 『{1}』开头", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.In)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 为: 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue:searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.Equal)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 等于 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.LikeEndAt)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 以 『{1}』结尾", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.LessThan)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 小于 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.LessThanOrEqual)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 小于等于 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.MoreThan)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 大于 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.MoreThanOrEqual)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 大于等于 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                    else if (searchInfo.SqlOperator == SqlOperator.NotEqual)
                    {
                        sb.AppendFormat(" 并且  〖{0}〗 不等于 『{1}』", searchInfo.FieldDescribe, string.IsNullOrWhiteSpace(searchInfo.FieldValueDEscribe) ? searchInfo.FieldValue : searchInfo.FieldValueDEscribe);
                    }
                }
            }

            describe += sb.ToString();
            describe = describe.Remove(0, 4);
            return describe;
        }
